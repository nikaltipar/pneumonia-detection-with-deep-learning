...
def identity_block(
    ...
    x = Conv2D(
        filters1, (1, 1), kernel_initializer='he_normal', kernel_regularizer=l2(l2_weight_decay)
    )(input_tensor)

    x = BatchNormalization(axis=bn_axis, momentum=batch_norm_decay, epsilon=batch_norm_epsilon)(x)
    ...
    x = Conv2D(
        filters3, (1, 1), kernel_initializer='he_normal', kernel_regularizer=l2(l2_weight_decay)
    )(x)
...
def resnet50(
    input_shape, l2_weight_decay, batch_norm_decay, batch_norm_epsilon, dropout_value, seed_value
):
