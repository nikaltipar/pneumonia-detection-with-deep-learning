...
data = util.readData(r'/home/nikaltipar/Desktop/')
...
model = md.resnet50((input_shape, input_shape, 3), l2_weight_decay, batch_norm_decay, batch_norm_epsilon, dropout_value, seed_value)
opt = Adam(lr=lr, decay=lr_decay)
model.compile(loss='binary_crossentropy', metrics=['accuracy'], optimizer=opt)

train_datagen = ImageDataGenerator(rescale=1./255, rotation_range=15, horizontal_flip=True, brightness_range=(0.9, 1.2))
test_val_datagen = ImageDataGenerator(rescale=1./255)


train_data_gen = train_datagen.flow_from_dataframe(train_data,
												   shuffle=True,
												   class_mode='binary',
...
train_data_gen_unaug = test_val_datagen.flow_from_dataframe(train_data,
														    class_mode='binary',
														    target_size=(input_shape, input_shape),
														    x_col='image', y_col='label',
														    batch_size=batch_size,
...
val_data_gen = test_val_datagen.flow_from_dataframe(val_data,
												    shuffle=False,
												    class_mode='binary',
												    target_size=(input_shape, input_shape),
												    x_col='image', y_col='label',
...
test_data_gen = test_val_datagen.flow_from_dataframe(test_data,
													 shuffle=False,
													 class_mode='binary',
													 target_size=(input_shape, input_shape),
													 x_col='image', y_col='label',
...
if len(old_model_filename) == 0:
    ...
    history = model.fit_generator(train_data_gen, class_weight=class_weight, epochs=nb_epochs, steps_per_epoch=nb_train_steps,
                                  validation_data=val_data_gen, validation_steps=val_data.shape[0]//batch_size)

    if nb_epoch_unaug > 0:
        history2 = model.fit_generator(train_data_gen_unaug, class_weight=class_weight, epochs=nb_epoch_unaug, steps_per_epoch=nb_train_steps,
                                  validation_data=val_data_gen, validation_steps=val_data.shape[0]//batch_size)
